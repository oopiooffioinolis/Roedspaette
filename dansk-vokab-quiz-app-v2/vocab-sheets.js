...
(function (root, factory) {
  ...
  const WORD_HEADERS = [
    ...
    "SourceURL", "DeepLink", "Note", "Status", "Due", "Interval", "Ease", "Reps", "Lapses"
  ];
  const PHRASE_HEADERS = [
    "ID", "DateAdded", "Phrase", "Gloss", "Translation", "Snippet",
    "SourceTitle", "SourceURL", "DeepLink", "Note", "Status",
    "Due", "Interval", "Ease", "Reps", "Lapses"
  ];
  // Spaced-repetition bookkeeping, hidden in Excel.
  const HIDDEN = new Set(["Due", "Interval", "Ease", "Reps", "Lapses"]);
  ...
  const WIDTHS = {
    ...
    DeepLink: 24, Note: 18, Status: 10, Due: 11, Interval: 9, Ease: 7,
    Reps: 6, Lapses: 7
  };
  ...
  /* Rows whose Status column equals "pending". Returns
     [{ sheet, row (0-based aoa index), type, term, id }]. */
  function listPending(wb) {
    const out = [];
    for (const [name, type, keyHeader] of [["Words", "word", "Term"], ["Phrases", "phrase", "Phrase"]]) {
      ...
      for (let r = 1; r < rows.length; r++) {
        if (String(rows[r][statusCol] || "").trim().toLowerCase() === "pending" && String(rows[r][keyCol] || "").trim()) {
          out.push({ sheet: name, row: r, type, term: String(rows[r][keyCol]).trim(), id: idCol >= 0 ? rows[r][idCol] : "" });
        }
      }
    }
    return out;
  ...
  return {
    ...
    ensureColumns, listPending, updateRow,
